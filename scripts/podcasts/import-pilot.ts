...
const PEOPLE = [
  { _id: "person.meyer-grunberg", name: "Meyer Grunberg", slug: "meyer-grunberg" },
  { _id: "person.isaac-simon", name: "Isaac Simon", slug: "isaac-simon" },
  { _id: "person.kalman-gavriel", name: "Kalman Gavriel", slug: "kalman-gavriel" },
  {
...
function importOwnedEpisode(
  ...
  return {
    ...
    chapters: episode.chapters.map((chapter) => ({
      _key: `${chapter.start}-${chapter.title}`.replace(/[^a-z0-9]+/gi, "-").slice(0, 48),
...
function episodeFingerprint(value: Record<string, unknown>) {
  ...
  const chapters = (value.chapters as { start?: string; title?: string }[]) || [];
  const guests = (value.guests as { _ref?: string }[]) || [];
  const hosts = (value.hosts as { _ref?: string }[]) || [];
  const links = (value.platformLinks as { platform?: string; identifier?: string }[]) || [];
  return {
    ...
    platformLinks: links.map((link) => `${link.platform}:${link.identifier}`).sort(),
  };
}
...
async function applyPilot() {
  ...
  const counts = {
    ...
    created: episodeResults.filter((item) => item.action === "created").length +
      (showResult.action === "created" ? 1 : 0),
    updated: episodeResults.filter((item) => item.action === "updated").length +
      (showResult.action === "updated" ? 1 : 0),
    skipped: episodeResults.filter((item) => item.action === "skipped").length +
...
async function main() {
  ...
  if (!apply) {
    throw new Error("Dry-run the RSS feed with podcasts:rss-dry-run. This command writes only with --apply --pilot.");
  }
