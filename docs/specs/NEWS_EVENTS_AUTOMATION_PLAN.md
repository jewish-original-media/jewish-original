...
Status: implemented on `feature/integration-homepage` from research
`research/news-events@128a537`. Architecture, public routes, Studio lists,
cron endpoints, and dry-run tooling are in Integration. Live AI publishing
and Sanity News/Event writes remain blocked until founder configuration.

The research-only banner below is historical. Do not treat it as the current
branch state.
...
| **Forward**                      | `https://forward.com/news/feed/`                                        | 20 items                                                       | Direct article URLs                    | Parseable          | Yes (~620 chars)                 | Free reprint of _original_ Forward stories under their pixel/canonical/noindex rules. V1 still links out                                                                     | Jewish World                           | **Yes**                      |
...
| Partner ICS #1–5     | ICS           | _to be supplied_          | mix of online / US / Israel / international | 2× daily             | Written permission to poll the feed and link to the official page            | Must have UID or URL, DTSTART, timezone | **Required before Events looks global** |
