...
const GUEST_TITLE_PATTERN =
  /^(?:sitting down with:\s*)?(.+?)\s+on\s+/i;

export function inferGuestNameFromTitle(title: string) {
  if (/^(season|premier|intro|feed drop|high holidays|yom kippur|pesach|purim|channukah|lag ba)/i.test(title)) {
    return undefined;
  }
...
function episodeFlags(episode: Omit<PodcastRssEpisode, "flags">): PodcastImportFlag[] {
  ...
  if (!episode.guid) {
    flags.push({
      ...
      field: "guid",
      severity: "blocking",
      evidence: "The RSS item has no guid, so an idempotent import key cannot be created.",
    });
  }
  if (!episode.audioUrl) {
    flags.push({
